...
CREATE TABLE IF NOT EXISTS rules (
  ...
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  name TEXT,
  conditions JSONB NOT NULL, -- Array of RuleCondition
  action JSONB NOT NULL, -- RuleAction
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
ALTER TABLE roi_tracker ENABLE ROW LEVEL SECURITY;
ALTER TABLE reminders ENABLE ROW LEVEL SECURITY;
ALTER TABLE expense_coverage ENABLE ROW LEVEL SECURITY;
...
-- Apply RLS policies to all user-owned tables
DO $$
DECLARE
  table_name TEXT;
BEGIN
...
-- Special policy for auth_tokens
CREATE POLICY "Users can manage own tokens" ON auth_tokens
